...
- auth-independent-tools：已安装的生图工具满足功能配置、provider 能力及模型要求时，API 登录与 ChatGPT 登录均可向模型暴露该工具；实际服务请求沿用原有凭据和授权流程。
...
工具登录方式解耦在客户端工具可见性判断处实现，保留独立的能力、功能开关和套餐限制。它不生成未安装的工具，也不改变后端认证或服务权限。
