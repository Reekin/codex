...
async fn assert_summary_response_is_non_fatal(summary_response: String) -> Result<()> {
    ...
    let mut builder = test_codex().with_chat_tree_summaries();
...
async fn resume_includes_initial_messages_from_rollout_events() -> Result<()> {
    ...
    match initial_messages.as_slice() {
        [
            EventMsg::ChatTreeNodeStarted(node_started),
            EventMsg::TurnStarted(started),
            EventMsg::UserMessage(first_user),
            EventMsg::AgentMessage(assistant_message),
            EventMsg::TokenCount(_),
            EventMsg::ChatTreeNodeFinalized(node_finalized),
            EventMsg::TurnComplete(completed),
        ] => {
            assert_eq!(node_started.node_id, started.turn_id);
            assert_eq!(
                node_started.turn_id.as_deref(),
            ...
            assert_eq!(node_finalized.node_id, started.turn_id);
            assert_eq!(node_finalized.status, ChatTreeNodeStatus::Completed);
