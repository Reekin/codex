...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    ...
    let first = tokio::time::timeout(std::time::Duration::from_millis(200), rx.recv())
        .await
        .expect("expected chat tree node started event without waiting for startup prewarm")
        .expect("channel open");
    assert!(matches!(
        first.msg,
    ...
    assert!(matches!(
        second.msg,
        EventMsg::TurnStarted(TurnStartedEvent { turn_id, trace_id, .. })
            if turn_id == tc.sub_id && trace_id == tc.trace_id
    ));
...
async fn interrupting_regular_turn_waiting_on_startup_prewarm_emits_turn_aborted() {
    ...
    let fourth = tokio::time::timeout(std::time::Duration::from_secs(2), rx.recv())
        .await
        .expect("expected chat tree node summary event")
        .expect("channel open");
    assert!(matches!(
        fourth.msg,
...
async fn build_initial_context_adds_multi_agent_v2_subagent_usage_hint_as_developer_message() {
    ...
    assert!(
        developer_messages
            .iter()
            .flatten()
            .any(|text| text.contains("your parent thread id is")),
        "expected subagent parent identity fact, got {developer_messages:?}"
    );
    assert!(
        !developer_messages
            .iter()
