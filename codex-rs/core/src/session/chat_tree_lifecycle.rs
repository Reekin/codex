...
impl Session {
    ...
    pub(crate) async fn start_chat_tree_node(
        ...
        let Some(started) = ({
            ...
            if turn_context.session_source.is_non_root_agent()
                && state.chat_tree.projection().nodes.is_empty()
            {
                return Ok(());
            }
    ...
    pub async fn set_current_chat_tree_node(
        ...
        self.state
            .lock()
            .await
            .replace_history_snapshot(selection.history);
        self.services
            .rollout_thread_trace
    ...
    pub(crate) async fn start_chat_tree_node_for_turn(
        ...
        let opens_chat_tree_node = input.iter().any(|item| match item {
            TurnInput::UserInput { content, .. } => !content.is_empty(),
            TurnInput::ResponseItem(response_item) => is_user_turn_boundary(&response_item.item),
            TurnInput::InterAgentCommunication(_) => true,
            TurnInput::FunctionCallOutput(_) => false,
        });
        if !opens_chat_tree_node {
            return;
        }
    ...
    pub(crate) async fn complete_chat_tree_node_before_turn_complete(
        ...
        let finalized_chat_tree_node = self
            ...
            .await;
        if !is_root_chat_tree_turn(turn_context)
            || !turn_context.config.chat_tree_summaries_enabled
