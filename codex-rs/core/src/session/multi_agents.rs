...
pub(crate) fn current_agent_name(session_source: &SessionSource, thread_id: ThreadId) -> String {
    session_source
        .get_agent_path()
        .map(String::from)
        .unwrap_or_else(|| match session_source {
            SessionSource::SubAgent(SubAgentSource::ThreadSpawn { .. }) => thread_id.to_string(),
            SessionSource::Cli
            | SessionSource::VSCode
            | SessionSource::Exec
            | SessionSource::Mcp
            ...
            | SessionSource::SubAgent(_) => "/root".to_string(),
        })
}
...
fn identity_hint_parts(session_source: &SessionSource) -> Option<(String, &'static str)> {
    ...
    let (mut text, assignment_text) = match agent_path {
        Some(agent_path) => (
            format!(
        ...
        None => (
            format!(
                "You are a spawned subagent in the multi-agent tree. You do not have a canonical agent path in this session, but you are still not the `/root` main agent. Your parent thread id is `{parent_thread_id}`, and your depth is {depth}."
            ),
