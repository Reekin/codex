...
pub(crate) fn create_exec_argv_tool_with_environment_id(
    ...
    let yield_time_ms_description = if cfg!(windows) {
        "Maximum time to wait before returning a session ID for a still-running command. Commands that finish sooner return immediately. Effective range on Windows is 10000-30000 ms."
    } else {
        "Wait before yielding output. Defaults to 10000 ms; effective range is 250-30000 ms."
    };
    let mut properties = BTreeMap::from([
        ...
        (
            "tty".to_string(),
            JsonSchema::boolean(Some(
                "True allocates a TTY for the command; false or omitted uses plain pipes."
                    .to_string(),
            )),
        ),
        (
