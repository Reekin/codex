...
async fn archive_thread(
    ...
    let file_name = validated_rollout_file_name(
    ...
    if let Some(ctx) = state_db_ctx
        && let Err(err) = ctx
            ...
            .await
    {
        if let Err(restore_err) = std::fs::rename(&archived_path, &canonical_rollout_path) {
...
mod tests {
    ...
    #[tokio::test]
    async fn archive_thread_moves_selected_rollout_and_updates_sqlite_metadata() {
        let home = TempDir::new().expect("temp dir");
