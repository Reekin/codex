...
impl ChatTreeOverlay {
    ...
    fn ensure_selected_visible(&mut self, visible_rows: usize) {
        if visible_rows == 0 || self.nodes.is_empty() {
            self.scroll_offset = 0;
            return;
        }
        if self.selected_idx < self.scroll_offset {
            self.scroll_offset = self.selected_idx;
        } else if self.selected_idx >= self.scroll_offset.saturating_add(visible_rows) {
            self.scroll_offset = self
                .selected_idx
                .saturating_add(1)
                .saturating_sub(visible_rows);
        }
        let max_offset = self.nodes.len().saturating_sub(visible_rows);
    ...
    fn render(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        Span::from("/ ".repeat(area.width as usize / 2))
            .dim()
            .render_ref(top, buf);
        " / C H A T  T R E E".dim().render_ref(top, buf);
        ...
        for row in 0..visible_rows {
            let Some(node) = self.nodes.get(self.scroll_offset.saturating_add(row)) else {
                break;
            };
            let idx = self.scroll_offset.saturating_add(row);
            let is_selected = idx == self.selected_idx;
            let is_current = self.current_node_id.as_ref() == Some(&node.node_id);
            let pointer = if is_selected { ">" } else { " " };
            let marker = if is_current { "[*]" } else { "[ ]" };
            let indent = "  ".repeat(node.depth);
            let text = format!("{pointer} {indent}{marker} {}", node.summary);
            let style = if is_selected {
                Style::default().bold()
            } else {
                Style::default()
            };
            Span::styled(text, style).render_ref(
                Rect::new(
                    content_area.x,
