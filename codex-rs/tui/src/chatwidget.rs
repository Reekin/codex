...
use codex_core::protocol::ChatTreeNodeUpdatedEvent;
use codex_core::protocol::ChatTreeTurnInfo;
...
impl ChatWidget {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::TurnStarted(ev) => {
                self.on_chat_tree_turn_started(&ev.turn_id);
            ...
            EventMsg::ChatTreeNodeUpdated(ChatTreeNodeUpdatedEvent { chat_tree }) => {
                self.on_chat_tree_turn_complete(Some(chat_tree));
                self.request_redraw();
            }
    ...
    pub(crate) fn chat_tree_overlay_entries(&self) -> Vec<ChatTreeOverlayEntry> {
        self.chat_tree_node_order
            .iter()
            .filter_map(|node_id| {
                let node = self.chat_tree_nodes.get(node_id)?;
                let summary = node
                    .summary
                    .clone()
                    .unwrap_or_else(|| "(pending summary)".to_string());
                Some(ChatTreeOverlayEntry {
                    node_id: node_id.clone(),
                    depth: self.chat_tree_depth(node_id),
                    summary,
    ...
    fn chat_tree_depth(&self, node_id: &str) -> usize {
