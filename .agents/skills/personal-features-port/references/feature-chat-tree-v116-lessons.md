...
Use `rebase/v0.116-chat-tree` as historical behavior and pitfall reference, not as a patch template.

Reference priority:

1. `feature-chat-tree-contract.md` and `feature-chat-tree-acceptance.md` define current required behavior.
2. The direct previous chat-tree feature branch is the primary code carry-forward reference.
3. Later branches that implemented completed-turn LLM summaries, such as `rebase/v0.128.0-chat-tree`, are better summary implementation references.
4. v116 is useful for understanding early complete behavior and recurring mistakes.
5. If v116 conflicts with the contract, follow the contract and record the deviation in the migration work package.
...
v116 uses an async model request after each completed turn to generate node summaries. Later release branches, including v0.128, preserve this as completed-port behavior.

Future ports may keep summary providers injectable so tests and early migration passes can use deterministic fallback or no-op providers. A completed port still must implement the v0.128-style async LLM summary path for completed normal turns with non-empty assistant output. Deterministic fallback is required for pending, aborted, empty-output, failed, or cancelled summary cases; it is not enough for feature parity by itself.
