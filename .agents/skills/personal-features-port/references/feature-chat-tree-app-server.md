...
- `expectedRevision`: integer or null, optional.
  - omitted: do not perform optimistic concurrency checking.
  - null: do not perform optimistic concurrency checking.
  - integer: reject the request if the current chat tree revision differs.

For app-server v2 request payloads, keep Rust `Option` and generated TypeScript optional/nullability aligned with the current protocol rules. Do not use `skip_serializing_if` on response fields to hide this value.
...
## App-Server Supporting Checks

Final migration acceptance is owned by `feature-chat-tree-acceptance.md`. Use these app-server-specific checks as supporting evidence when the app-server protocol, schema, notifications, or selected-branch transcript behavior is touched:
