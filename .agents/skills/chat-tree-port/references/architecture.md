...
Keep chat tree lifecycle integration behind a small session-level adapter instead of spreading feature rules across task orchestration.

Goal:

- future migrations should usually rewire start/completion/abort hook calls, not rediscover chat tree summary, cancellation, and persistence ordering;
- task code should call high-level chat tree lifecycle methods at turn start, completed-turn finalization, and abort finalization;
- summary provider details should live with the lifecycle adapter, not in generic task scheduling code.

Required lifecycle shape:

- start hook captures the new node parent from the current selected branch before the model request can create a new branch;
- lifecycle eligibility is centralized and excludes all non-root session sources, including internal side threads and subagents;
- completed-turn hook finalizes the node and registers the per-node summary cancellation token before `TurnComplete` is emitted;
- the async LLM summary request is spawned only after `TurnComplete` is emitted, so user-visible turn completion is never blocked by summary generation;
- shutdown and session-loop exit can cancel summary work that has been registered but not yet spawned;
- summary persistence is cancellation-safe: once durable update begins, rollback/event delivery is not left half-applied by racing cancellation;
- abort hook finalizes interrupted/replaced/review-ended nodes and persists the deterministic fallback label.

Acceptance for this adapter:

- normal completed turns still produce durable nodes and required asynchronous LLM summaries;
- non-root turns, including internal side threads and subagents, remain excluded unless the product contract changes;
- late summary updates do not change model-visible history, current node, or visible turn IDs;
- replay still restores current-node model-visible history from the selected branch rather than linear rollout order;
- focused tests cover summary request shape, truncation, cancellation-before-persistence, registered-before-spawn cancellation, and selected-branch next request bodies.
...
- `ModelSummaryProvider`: required async LLM summary for completed normal turns with non-empty assistant output.
- `NoopSummaryProvider`: useful only during an early migration pass, not in a completed port.

The feature may work temporarily with deterministic/noop summary while branch semantics are being restored, but a completed port must include the model summary provider and its persistence/replay tests.
