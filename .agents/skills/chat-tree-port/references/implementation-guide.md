...
- normal user turns create durable tree nodes;
- current-node switching changes future model context without deleting branches;
- selected-branch transcript and model-visible context agree;
- restart/replay restores nodes, current node, summaries, and visible branch;
...
1. try to carry forward the shared domain, app-server wire shapes, fixtures, and tests by rebase or direct copy;
...
In both modes, the copyable domain/API/projection/test contract should remain stable. The difference is only whether Git can carry those pieces forward mechanically or the agent needs to place them into the new architecture by hand.
...
## Phase 0: Write The Implementation Brief

Before changing code, write a short brief in your working notes. The brief prevents later review loops from turning into architecture churn.

Required fields:

- **Upstream base**: tag/commit and branch name.
- **Supported UI paths**: app-server TUI, native TUI, both, or redirected paths.
- **Persistence source**: rollout, sidecar chat-tree log, or hybrid.
- **Stable app-server contract**: method names, response fields, notification shape, and error `data.kind` values.
- **Summary scope**: deterministic fallback only, async/model summary, or explicit future work.
- **Rollback/compaction strategy**: support, reject, or map to tree rebuild/prune events.
- **Runtime context strategy**: full snapshots as cache, ancestor-turn rebuild, deltas, or shared-prefix cache.
- **Validation commands**: focused tests and schema/snapshot commands expected for this port.

Do not implement until this brief identifies the active session, turn, replay, app-server, and UI hook points.
...
Branch correctness does not depend on model summaries.
...
- mutate node context;
- change current node;
- change visible turns.

If only fallback labels are implemented, document async/model summary as future work rather than exposing it as complete behavior.

## Phase 8: Validate In Dependency Order

Run tests in this order so failures point to the right layer:

1. domain reducer/projection tests;
2. core golden context test: A -> B -> C, switch A, send D, verify model request excludes B/C;
3. replay tests: restart, compaction, rollback/prune, stale revisions, missing parent/current diagnostics;
4. app-server read/set-current tests;
5. app-server notification and wire fixture tests;
6. `thread/read` and `thread/turns/list` selected-branch tests;
7. TUI overlay tests;
8. supported UI transcript-refresh tests;
9. schema generation, snapshots, formatting, scoped lint/fix;
10. final diff review for lockfile and unrelated churn.
...
Classify every review item:

- **P0 contract break**: behavior, persistence, replay, app-server contract, model context, or supported UI is wrong. Fix now.
- **P1 portability hardening**: the implementation works, but a targeted change reduces future rebase risk. Fix when small and testable.
- **P2 long-term debt**: memory optimization, cleaner abstractions, or future feature polish. Document without blocking completion.

Stop iterating when:

- all P0 items are fixed;
- supported golden scenarios pass;
- app-server stable fixtures pass;
- remaining P1/P2 items are documented;
- the implementation no longer contains duplicate tree/projector logic.
